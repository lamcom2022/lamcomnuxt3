  ...
  <div>
    ...
    >
      ...
      <div class="hidden lg:block relative lg:w-3/4 lg:h-full mx-auto">
        <img
          class="place-items-center mx-12"
          src="https://res.cloudinary.com/lamcom/image/upload/c_scale,w_900/v1663429460/mkagency/login/login_brrler.jpg"
        />
      </div>
      ...
      <div class="lg:mx-auto lg:w-full py-40 items-left">
        ...
        <p
          v-if="!forgotPassword && !formSignUp"
...
export default {
  ...
  async signIn() {
      try {
        if (this.formSignUp == true) {
          try {
            if (this.data.email == undefined && this.data.phone == undefined) {
              this.error = "Please enter Email or Phone Number";
            } else if (
              this.data.password != undefined &&
              this.data.confirm_password != undefined
              //&& this.data.termsandcondition != undefined
            ) {
              this.notSamePassword = true;
              this.error = "";
              let request = {};
              console.log(this.data);
              const { data: user } = await useFetch("/api/signin", {
                method: "post",
                body: this.data,
              });

              console.log("user", user);
              if (user._rawValue.document.email !== this.data.email) {
                const { data: SignUp } = await useFetch("/api/signup", {
                  method: "post",
                  body: this.data,
                });
        ...
        if (this.formSignIn == true) {
          try {
            if (this.data.email != "" && this.data.password != "") {
              const { data: userverify, refresh } = await useFetch(
                "/api/signin",
                {
                  method: "post",
                  body: this.data,
              ...
              if (userverify._rawValue.document !== null) {
                if (
                  userverify._rawValue.document.email === this.data.email &&
                  userverify._rawValue.document.password === this.data.password
                ) {
                  this.data = {};
                  return await navigateTo({
