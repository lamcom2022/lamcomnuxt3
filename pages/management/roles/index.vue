					...
					<div class="flex-1 flex flex-col overflow-y-auto xl:overflow-hidden">
						...
						<div class="flex-1 flex xl:overflow-hidden">
							...
							<div class="flex-1 xl:overflow-y-auto">
								<RolePageHeader :heading="pageHeading" showbutton="true" guide="Manage roleship subscriptions i.e. search, add/modify/delete/activate/de-activate roleships" addClickEvent="evtUpsertRole" />
								<RoleSearch :heading="searchHeading" />
								<div class="w-full">
									<!-- Main content (start)-->

									<!-- Role Body content (start)-->
...
<script>
	import RolePageHeader from "@/pages/management/roles/RolePageHeader.vue";
	import RoleSearch from "@/pages/management/roles/RoleSearch.vue";
	import RoleDataTable from "@/pages/management/roles/RoleGrid.vue";

	import RoleUpsert from "@/pages/management/roles/RoleUpsert.vue";
	...
	export default {
		...
		methods: {
			async getrole(email) {
				try {
					/*let api = 'Role?email=' + email;
		const { data } = await this.$axios.get(api);
		console.log(JSON.stringify(data));
		//alert(JSON.stringify(data.data[0]))
		return (this.role = data.data[0]);*/
				} catch (error) {
					console.log(error);
				}
			},
		...
		created() {
			//this.getrole(this.$route.params.slug);
		},
	};
</script>
<script setup>
